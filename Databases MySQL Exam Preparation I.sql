...
SELECT a.airline_id	,a.airline_name,	a.nationality,	a.rating
   FROM (SELECT  airline_id	, airline_name,nationality,	rating FROM airlines
...
ORDER BY ti.price DESC, c.customer_id ASC
LIMIT 3;
...
CREATE PROCEDURE udp_submit_review(customer_id1  INT, review_content1 VARCHAR(255),
  ...
  BEGIN
    ...
    SET airline_id2 := (SELECT a.airline_id FROM airlines AS a WHERE a.airline_name LIKE airline_name1);
    IF airline_id2 IS NULL
    THEN
